...
static uint32_t shooter_audio_gpio[] = {
	GPIO_CFG(SHOOTER_AUD_CDC_LDO_SEL, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};

void shooter_audio_gpios_init(void)
{
	pr_aud_info("%s\n", __func__);
	gpio_tlmm_config(shooter_audio_gpio[0], GPIO_CFG_DISABLE);
