...
static struct perflock_data shooter_perflock_data = {
	.perf_acpu_table = shooter_perf_acpu_table,
	.table_size = ARRAY_SIZE(shooter_perf_acpu_table),
};

static struct perflock_data shooter_cpufreq_ceiling_data = {
	.perf_acpu_table = shooter_perf_acpu_table,
	.table_size = ARRAY_SIZE(shooter_perf_acpu_table),
};

static struct perflock_pdata perflock_pdata = {
       .perf_floor = &shooter_perflock_data,
       .perf_ceiling = &shooter_cpufreq_ceiling_data,
};
...
static uint32_t mhl_gpio_table[] = {
	...
	GPIO_CFG(SHOOTER_GPIO_MHL_WAKE_UP, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};
...
static struct dsps_gpio_info dsps_shooter_gpios[] = {
...
static void __init msm8x60_init_dsps(void)
{
	...
	pdata->gpios = dsps_shooter_gpios;
	pdata->gpios_num = ARRAY_SIZE(dsps_shooter_gpios);
...
static struct msm_rtb_platform_data shooter_rtb_pdata = {
...
static int __init msm_rtb_set_buffer_size(char *p)
{
	...
	s = memparse(p, NULL);
	shooter_rtb_pdata.size = ALIGN(s, SZ_4K);
	return 0;
}
early_param("msm_rtb_size", msm_rtb_set_buffer_size);


static struct platform_device shooter_rtb_device = {
	.name           = "msm_rtb",
	.id             = -1,
	.dev            = {
		.platform_data = &shooter_rtb_pdata,
	},
};
#endif
...
{
#ifdef CONFIG_I2C
	//pm8901_vreg_init[PM8901_VREG_ID_MPP0].active_high = 1;
#endif
}
...
static uint32_t __initdata shooter_regulator_lpm_set[] =
{
...
static void __init msm8x60_init(struct msm_board_data *board_data)
{
	...
	msm_rpm_lpm_init(shooter_regulator_lpm_set, ARRAY_SIZE(shooter_regulator_lpm_set));
