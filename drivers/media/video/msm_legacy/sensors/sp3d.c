...
static int g_antibanding_type = -1;
static enum aec_awb_lock_mode g_aec_awb_lock_mode = UNLOCK_AEC_UNLOCK_AWB_MODE;
...
#define SP3D_CHECK_SPI(a) ({\
	if (a < 0) \
		pr_err("[CAM] %s rc:%d line:%d", __func__, a, __LINE__); \
	a; \
})
...
static int sp3d_spi_read(uint16_t reg, uint16_t *rval)
{
	...
	rc = sp3d_spi_sync_write_then_read(&rx[0], 3, &rx[3], 3);
#if 0
	pr_info("[CAM] %s: rc = %d\n", __func__, rc);
	pr_info("[CAM] rx[3] = 0x%x, rx[4] = 0x%x, rx[5] = 0x%x\n",
		rx[3], rx[4], rx[5]);
#endif
	if (rc >= 0)
		*rval = rx[4] << 8 | rx[5];
...
static int sp3d_spi_write(uint16_t reg, uint16_t val)
{
	...
	rc = sp3d_spi_single_write(&wb[0], 3);
	return rc;
}
					 ...
					 *reg_conf_tbl, int orig_num)
{
 ...
 SP3D_KEEPWRITE:
    ...
    if (!sp3d_spi_write_addr) {
		pr_err("[CAM] Error allocating memory retrying num:%d\n", orig_num);
		return -1;
    }
    spi_message_init(&m);
    memset(&tx_addr, 0, sizeof(struct spi_transfer));
...
static void sp3d_config_csi_param(struct msm_camera_csi_params *sp3d_csi_params)
{
	...
	sp3d_lane_cnt = sp3d_csi_params->lane_cnt;
}
...
static int sp3d_3D_get_L_frame(void)
{
	...
	msm_camio_csi_enable_lp_rec(sp3d_lane_cnt);
	return 0;
}
...
#endif
	/*LP00*/
	msm_camio_csi_disable_lp_rec(sp3d_lane_cnt);
...
SP3D_RECHECKVALUE:
	...
	if (val == 0x8f) {
		...
		if (sp3d_check_count > 20) {
			...
			pr_info("[CAM]sp3d value check NACK :%d sp3d_check_count:%d", command, sp3d_check_count);
			sp3d_wait_INT(1);
			goto SP3D_RECHECKVALUE;
		}
	} else {
...
int sp2d_init_setting_thread(void)
{
	...
	#if 0 /*take off due to 2D mode dosent need this setting suggest by sharp*/
	...
	if (atomic_read(&snapshot_flag)) {
		if (atomic_read(&sp3d_snapshot_state) != SP3D_2DSNAPSHOT_NOT_START ||
			...
			pr_info("[CAM]----------Got INT from sharp state3D:%d state2D:%d---------------",
				atomic_read(&sp3d_snapshot_state), atomic_read(&sp3d_2Dsnapshot_state));
		}
	}
...
static int32_t sp3d_sensor_setting(int update_type, int rt)
{
	...
	case UPDATE_PERIODIC:/*3D*/
		if (!config_csi) {
			...
			msm_camio_csi_enable_lp_rec(sp3d_lane_cnt);
			sp3d_set_antibanding(g_antibanding_type);
			break;
		}
		sp3d_wait_INT(1);
		if (rt == RES_CAPTURE) {
...
int sp3d_spi_open_init(struct msm_camera_sensor_info *data)
{
	...
	pr_info("[CAM]%s: %d\n", __func__, __LINE__);
	if (data == NULL) {
		pr_err("[CAM] data pointer is NULL");
		return -1;
	}
	...
	if (sp3d_spi_write_addr == NULL) {
		...
		pr_info("[CAM] sp3d_spi_write_addr start:%p end:%p",
			sp3d_spi_write_addr, (sp3d_spi_write_addr + SP3D_MAX_ALLOCATE * 8));
	}
	pr_info("[CAM] sp3d_spi_write_addr:0x%p", sp3d_spi_write_addr);
	rc = sp3d_probe_init_sensor(data);
	if (rc < 0) {
		CDBG("[CAM] Calling sp3d_spi_open_init fail\n");
		goto init_fail;
	}
...
init_fail:
	CDBG("[CAM] sp3d_spi_open_init fail\n");
	sp3d_probe_init_done(data);
	kfree(sp3d_ctrl);
	sp3d_ctrl = NULL;
init_done:
	CDBG("[CAM] sp3d_spi_open_init done\n");
	return rc;
}
...
static int sp3d_get_af_state(void)
{
	...
	if (temp)
		...
		pr_info("[CAM]sp3d lens not moving");
...
static int sp3d_set_effect(int8_t effect_type)
{
	...
	case CAMERA_EFFECT_WHITEBOARD:
		/*workaround to fix ATS set effect issue*/
		rc = true;
		break;
...
static int sp3d_get_iso(uint16_t *real_iso_value)
{
    ...
    *real_iso_value = AgcGain * 32 / 16;
    pr_info("[CAM] %s real_iso_value: %d AgcGain:%d", __func__, *real_iso_value, AgcGain);
    return rc;
}
...
static int sp3d_contrast_remap(enum contrast_mode contrast_type)
{
    uint8_t rc = -1;
    switch(contrast_type) {
	case CAMERA_CONTRAST_N2:
...
static int sp3d_sharpness_remap(enum sharpness_mode sharpness_type)
{
    uint8_t rc = -1;
    switch(sharpness_type) {
    case CAMERA_SHARPNESS_X0:
...
static int sp3d_saturation_remap(enum saturation_mode saturation_type)
{
    uint8_t rc = -1;
    switch(saturation_type) {
	case CAMERA_SATURATION_X0:
...
static int sp3d_set_image_quality(enum contrast_mode contrast_type,		/* parameter range is 0xfe - 0x02.*/
                                      enum sharpness_mode sharpness_type,	/* parameter range is 0xfc - 0x04.*/
                                      enum saturation_mode saturation_type)	/* parameter range is 0xfe - 0x02.*/
{
    int rc =0;
    if(atomic_read(&snapshot_flag) || !atomic_read(&preview_flag))
	    return rc;
	sp3d_wait_INT(1);
	sp3d_spi_write(0x0492,0x33F8);		/* access to master camera */
	sp3d_spi_write(0x049A,0x020E);
	sp3d_spi_write(0x0490,0x0402);
	sp3d_spi_write(0x049A,0x0000);
	sp3d_spi_write(0x0490,0x0402);
	sp3d_spi_write(0x049A,0x0500);      /* 0E00 : luminance */
	sp3d_spi_write(0x0490,0x0402);
	sp3d_spi_write(0x049A,0x0500 | sp3d_contrast_remap(contrast_type));		/* 0E01 : contrast */
	sp3d_spi_write(0x0490,0x0402);
	sp3d_spi_write(0x049A,0x0500 | sp3d_sharpness_remap(sharpness_type));	/* 0E02 : Sharpness */
	sp3d_spi_write(0x0490,0x0402);
	sp3d_spi_write(0x049A,0x0500 | sp3d_saturation_remap(saturation_type));	/* 0E03 : Color saturation */
	sp3d_spi_write(0x0490,0x0402);
	sp3d_spi_write(0x049A,0x0500);					                        /* 0E04 */
	sp3d_spi_write(0x0490,0x0402);
	sp3d_spi_write(0x049A,0x020D);
	sp3d_spi_write(0x0490,0x0402);
	sp3d_spi_write(0x049A,0x000D);
	sp3d_spi_write(0x0490,0x0402);
	sp3d_spi_write(0x049A,0x054C);	   /* 0D0D : 4C */
	sp3d_spi_write(0x0490,0x0402);
	sp3d_spi_write(0x0492,0x37F8);	   /* access to slave camera */
	sp3d_spi_write(0x049A,0x020E);
	sp3d_spi_write(0x0490,0x0402);
	sp3d_spi_write(0x049A,0x0000);
	sp3d_spi_write(0x0490,0x0402);
...
static int sp3d_set_aec_awb_lock_mode(enum aec_awb_lock_mode lock_mode)
 {
    int rc = 0;
	if (atomic_read(&snapshot_flag) || !atomic_read(&preview_flag))
		return rc;
	sp3d_wait_INT(1);
	pr_info("[CAM]%s: lock_type %d", __func__, lock_mode);
	switch (lock_mode) {
	case UNLOCK_AEC_UNLOCK_AWB_MODE:
		rc = sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_unlock_aec_unlock_awb,
									sp3d_com_feature_regs.reg_koj_unlock_aec_unlock_awb_size);
		g_aec_awb_lock_mode = UNLOCK_AEC_UNLOCK_AWB_MODE;
		break;
	case UNLOCK_AEC_LOCK_AWB_MODE:
		rc = sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_unlock_aec_lock_awb,
									sp3d_com_feature_regs.reg_koj_unlock_aec_lock_awb_size);
		g_aec_awb_lock_mode = UNLOCK_AEC_LOCK_AWB_MODE;
		break;
	case LOCK_AEC_UNLOCK_AWB_MODE:
		rc = sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_lock_aec_unlock_awb,
									sp3d_com_feature_regs.reg_koj_lock_aec_unlock_awb_size);
		g_aec_awb_lock_mode = LOCK_AEC_UNLOCK_AWB_MODE;
		break;
	case LOCK_AEC_LOCK_AWB_MODE:
		rc = sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_lock_aec_lock_awb,
									sp3d_com_feature_regs.reg_koj_lock_aec_lock_awb_size);
		g_aec_awb_lock_mode = LOCK_AEC_LOCK_AWB_MODE;
		break;
	default:
	    pr_info("[CAM]%s:unknown lock mode", __func__);
		break;
	}
	return rc;
}

static int sp3d_set_scene_mode(enum bestshot_mode bestshot_mode)
{
	int rc = 0;
	...
	pr_info("[CAM]%s, bestshot_mode: %d", __func__, bestshot_mode);

	switch (bestshot_mode) {
	case BESTSHOT_LANDSCAPE:
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_wb_auto,
					sp3d_com_feature_regs.reg_koj_wb_auto_size);			/* WB:Auto */
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_iso_100,
					sp3d_com_feature_regs.reg_koj_iso_100_size);			/* ISO: 100 */
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_af_mode_normal,
					sp3d_com_feature_regs.reg_koj_af_mode_normal_size);		/* AF: Normal */
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_ev_0,
					sp3d_com_feature_regs.reg_koj_ev_0_size);			/* EV: Default */
		sp3d_set_image_quality(CAMERA_CONTRAST_P1,						/* Contrast: P1 */
					CAMERA_SHARPNESS_X3,						/* Sharpness: X3  */
					CAMERA_SATURATION_X15);						/* Saturation: X15*/
		break;
	case BESTSHOT_CANDLELIGHT:
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_wb_incandescent,
					sp3d_com_feature_regs.reg_koj_wb_incandescent_size);		/* WB: Incandescent Light */
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_iso_auto,
					sp3d_com_feature_regs.reg_koj_iso_auto_size);			/* ISO: Auto */
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_af_mode_allrange,
					sp3d_com_feature_regs.reg_koj_af_mode_allrange_size);		/* AF: Auto */
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_ev_neg2,
					sp3d_com_feature_regs.reg_koj_ev_neg2_size);			/* EV: N2 */
		sp3d_set_image_quality(CAMERA_CONTRAST_D,						/* Contrast: Default */
					CAMERA_SHARPNESS_X2,						/* Sharpness: Default  */
					CAMERA_SATURATION_X1);						/* Saturation: Default */
		break;
	case BESTSHOT_PORTRAIT:
		...
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_ev_0,
					sp3d_com_feature_regs.reg_koj_ev_0_size);			/* EV: Default  */
		sp3d_set_image_quality(CAMERA_CONTRAST_P1,						/* Contrast: P1*/
					CAMERA_SHARPNESS_X1,						/* Sharpness: X1 */
					CAMERA_SATURATION_X15);						/* Saturation: X15*/
		break;
	case BESTSHOT_ACTION:
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_wb_auto,
					sp3d_com_feature_regs.reg_koj_wb_auto_size);			/* WB:Auto */
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_iso_800,
					sp3d_com_feature_regs.reg_koj_iso_800_size);			/* ISO: 800 */
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_af_mode_allrange,
					sp3d_com_feature_regs.reg_koj_af_mode_allrange_size);		/* AF: Auto */
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_ev_neg2,
					sp3d_com_feature_regs.reg_koj_ev_neg2_size);			/* EV: N2 */
		...
		break;
	case BESTSHOT_BEACH:
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_wb_daylight,
					sp3d_com_feature_regs.reg_koj_wb_daylight_size);		/* WB: Outdoor */
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_iso_100,
					sp3d_com_feature_regs.reg_koj_iso_100_size);			/* ISO: 100 */
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_af_mode_allrange,
					sp3d_com_feature_regs.reg_koj_af_mode_allrange_size);		/* AF: Auto */
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_ev_1,
					sp3d_com_feature_regs.reg_koj_ev_1_size);			/* EV: P1 */
		sp3d_set_image_quality(CAMERA_CONTRAST_D,						/* Contrast: Default */
					CAMERA_SHARPNESS_X2,						/* Sharpness: Default */
					CAMERA_SATURATION_X15);						/* Saturation: X15*/
		break;
	case BESTSHOT_SNOW:
		...
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_af_mode_allrange,
					sp3d_com_feature_regs.reg_koj_af_mode_allrange_size);		/* AF: Auto */
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_ev_2,
					sp3d_com_feature_regs.reg_koj_ev_2_size);			/* EV: P2 */
		sp3d_set_image_quality(CAMERA_CONTRAST_D,						/* Contrast: Default */
					CAMERA_SHARPNESS_X2,						/* Sharpness: Default */
					CAMERA_SATURATION_X1);						/* Saturation: Default */
		break;
	case BESTSHOT_FLOWERS:
		...
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_iso_auto,
					sp3d_com_feature_regs.reg_koj_iso_auto_size);			/* ISO: Auto */
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_af_mode_macro,
					sp3d_com_feature_regs.reg_koj_af_mode_macro_size);		/* AF: Marco */
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_ev_0,
					sp3d_com_feature_regs.reg_koj_ev_0_size);			/* EV: Default */
		sp3d_set_image_quality(CAMERA_CONTRAST_D,						/* Contrast: Default */
					CAMERA_SHARPNESS_X4,						/* Sharpness: X4 */
					CAMERA_SATURATION_X1);						/* Saturation: Default */
		break;
	case BESTSHOT_NIGHT:
		sp3d_spi_write_table2(sp3d_com_feature_regs.reg_koj_wb_auto,
...
{
	uint16_t rval = 0;
	pr_info("[CAM][3D calibration] switch to slave");
	...
	while (1) {
		...
		if (rval == 0x80) {
			pr_info("[CAM][3D calibration]slave ready");
			break;
		}
	}
}
...
{
	pr_info("[CAM][3D calibration]otp_data.coefA1:0x%llx", otp_data->coefA1);
	pr_info("[CAM][3D calibration]otp_data.coefB1:0x%llx", otp_data->coefB1);
	pr_info("[CAM][3D calibration]otp_data.coefC1:0x%llx", otp_data->coefC1);
	pr_info("[CAM][3D calibration]otp_data.coefA2:0x%llx", otp_data->coefA2);
	pr_info("[CAM][3D calibration]otp_data.coefB2:0x%llx", otp_data->coefB2);
	pr_info("[CAM][3D calibration]otp_data.coefC2:0x%llx", otp_data->coefC2);
	pr_info("[CAM][3D calibration]otp_data.coefA3:0x%llx", otp_data->coefA3);
	pr_info("[CAM][3D calibration]otp_data.coefB3:0x%llx", otp_data->coefB3);
	pr_info("[CAM][3D calibration]otp_data.coefC3:0x%llx", otp_data->coefC3);
}
...
static void sp3d_read_OTP(struct Sp3d_OTP *otp_data, struct otp_cfg *sp3d_otp_cfg)
{
	...
	while (count < 0x50) {
		...
		sp3d_spi_write(0x490, 0x0006);
		sp3d_spi_read(0x4A2, &rval);
		/* pr_info("[CAM][3D calibration]rval:0x%x",rval); */
		if (count == 0x48) {
			otp_version = rval;
			sp3d_otp_cfg->sp3d_otp_version = otp_version;
			pr_info("[CAM][3D calibration]otp_version:0x%x", otp_version);
		}
	...
	sp3d_spi_read(0x4A2, &rval);
	id_avaliabe = rval;
	pr_info("[CAM][3D calibration]id_avaliabe:0x%x", id_avaliabe);
	if (id_avaliabe == 0x80)
		pr_info("[CAM][3D calibration]id is avaliabe");
	if (id_avaliabe == 0x81)
		pr_info("[CAM][3D calibration]id is not avaliabe");
	...
	while (count <= 0x005E) {
		...
		pr_info("[CAM][3D calibration]Module ID:0x%x,count 0x%x", rval, count);
		memcpy(sp3d_otp_cfg->sp3d_id, otp_id, sizeof(otp_id));
		count++;
	}
	pr_info("[CAM][3D calibration]read OTP finish");
}
...
static void sp3d_calibration_ready(void)
{
	...
	while (1) {
		...
		sp3d_spi_read(0x04A2, &rval);
		if (rval == 0x80) {
			pr_info("[CAM][3D Calibration]master ready");
			break;
		}
		if (count > 200) {
			pr_info("[CAM][3D Calibration]%s check reg time out", __func__);
			break;
		}
	}
}
...
static int sp3d_set_calibration(struct otp_cfg *sp3d_otp_cfg)
{
	...
	if (rc < 0) {
		pr_info("[CAM][SP3D calibration] reg_para1_koj_mixer_cal_3d");
		goto faile;
	}
	udelay(100);
	...
	return rc;
faile:
	pr_err("[CAM][SP3D calibration]calibration setting faile");
       return rc;
}
...
static uint16_t sp3d_read_Ext_mode(uint16_t mode)
{
	...
	sp3d_spi_write(0x0490, 0x0006);
	sp3d_spi_read(0x04A2, &rval);
	/* pr_info("[CAM] %s mode:0x%x rval:0x%x",__func__,mode,rval); */
	return rval;
}
...
static int sp3d_change_framerate(struct sensor_cfg_data *cdata)
{
	...
	pr_info("[CAM] sp3d_change_framerate cdata->cfg.down_framerate:%d", cdata->cfg.down_framerate);
	pr_info("[CAM] sp3d_change_framerate cdata->mode:%d", cdata->mode);
	switch (cdata->mode) {
	case SENSOR_VIDEO_MODE:
		if (cdata->cfg.down_framerate == 1) {
			pr_info("[CAM] %s 2D video mode 15fps", __func__);
			rc = sp3d_spi_write_table2(sp3d_2D_regs.reg_koj_2d_720p_video_15fps,
									sp3d_2D_regs.reg_koj_2d_720p_video_15fps_size);
		} else {
			pr_info("[CAM] %s 2D video mode 24fps", __func__);
	...
	case SENSOR_PREVIEW_MODE:
		...
		break;
	default:
		pr_err("[CAM] %s wrong mode", __func__);
		break;
	}
	return rc;
}
...
static int sp3d_get_exp_gain(struct exp_cfg *exp_data)
{
	...
	sp3d_spi_write(0x0490, 0x0006);
	sp3d_spi_read(0x04A2, &rval);
	pr_info("[CAM] flicker_compansation:%0x", rval);
...
static int sp3d_set_flashlight(int flash_mode, struct flash_cfg *flash_data)
{
	...
	if (flash_mode == 0) { /*flash off*/
		pr_info("[CAM] sp3d_set_flashlight camera_flash FL_MODE_OFF");
		sdata->flash_data->flash_src->camera_flash(FL_MODE_OFF);
		pr_info("[CAM] sp3d_set_flashlight done");
...
int sp3d_spi_config_temp(void *data)
{
	...
	case CFG_SET_MODE:
		...
		rc = sp3d_set_sensor_mode(cdata->mode, cdata->rs);
		break;
...
static enum sharpness_mode  g_img_sharpness = CAMERA_SHARPNESS_X2;
static enum saturation_mode g_img_saturation = CAMERA_SATURATION_X1;
static enum contrast_mode	g_img_contrast = CAMERA_CONTRAST_D;
...
int sp3d_spi_config(void __user *argp)
{
	...
	case CFG_SET_SHARPNESS:
		g_img_sharpness = cdata.cfg.sharpness_value;
		rc = sp3d_set_image_quality(g_img_contrast,
			                        g_img_sharpness,
			                        g_img_saturation);
		break;
	case CFG_SET_SATURATION:
		g_img_saturation = cdata.cfg.saturation_value;
		rc = sp3d_set_image_quality(g_img_contrast,
			                        g_img_sharpness,
			                        g_img_saturation);
		break;
	case CFG_SET_CONTRAST:
		g_img_contrast = cdata.cfg.contrast_value;
		rc = sp3d_set_image_quality(g_img_contrast,
			                        g_img_sharpness,
			                        g_img_saturation);
	...
	case CFG_SET_SCENE_MODE:
		rc = sp3d_set_scene_mode(cdata.cfg.bestshot_mode_value);
		break;
    case CFG_SET_AEC_AWB_LOCK_MODE:
		rc = sp3d_set_aec_awb_lock_mode(cdata.cfg.aec_awb_lock_mode_value);
		break;
	default:
		rc = -EFAULT;
		break;
...
#if 0
probe_failure:
	CDBG("[CAM] sp3d_spi_probe failed! rc = %d\n", rc);
	return rc;
#endif
}
