...
static void vfe31_stop(void)
{
	...
	unsigned long flags;
	if (vfe31_ctrl == NULL) {
		pr_err("[CAM] vfe31_stop vfe31_ctrl pointer is NULL");
		return ;
	}
...
static int vfe31_proc_general(struct msm_vfe31_cmd *cmd)
{
	...
	switch (cmd->id) {
	case V31_RESET:
		pr_info("[CAM] vfe31_proc_general: V31_RESET\n");
		vfe31_reset();
		break;
	case V31_START:
	...
	case V31_UPDATE:
		...
		break;
	case V31_CAPTURE:
		pr_info("[CAM] vfe31_proc_general: cmdID = V31_CAPTURE\n");
	...
	*/
	...
	if (out_bool) {
		...
		if (vfe31_ctrl->outpath.out0.frame_drop_cnt > 20) {
			pr_err("[CAM] path_irq_0 - no free buffer!\n");
